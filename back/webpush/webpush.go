...
type VAPID struct {
	PublicKey  string    // PublicKey, passed in VAPID Authorization header
	PrivateKey string    // PrivateKey, used to sign VAPID JWT token
	Expiration time.Time // Expiration optional expiration for VAPID JWT token (defaults to now + 12 hours)
}
...
func SendNotificationWithContext(ctx context.Context, message []byte, s *Subscription, options *Options) (*http.Response, error) {
	/*
		TODO: revisit implementation. source: https://github.com/SherClockHolmes/webpush-go
		 Examples by Mozilla: https://github.com/web-push-libs/web-push
	*/
	...
	expiration := options.VAPID.Expiration
...
func decodeSubscriptionKey(key string) ([]byte, error) {
	...
	bts, err := base64.StdEncoding.DecodeString(buf.String())
	if err == nil {
		return bts, nil
	}
